...
Beyond the five core anomaly types, the framework's rule engine and SE detectors provide partial coverage for 20 extended anomaly subtypes (Table~\ref{tab:confusion}). Among 25 subtypes tested across 39 networks, 6 achieve $\geq$90\% recall (grounding fault, clock drift, voltage regulation, topo interruption, stale data, virtual/faulty connection), while 8 receive no coverage (model mismatch, signal mismatch, communication loss, ghost topology, duplicate measurement, harmonic pollution, DG intermittent, branch contingency). The overall recall across 25 subtypes is 46.8\%. Partial-coverage subtypes (load shift: 76.3\%, reverse power flow: 68.4\%, parameter error: 61.0\%, protection misconfig: 58.4\%) represent the most promising targets for future rule engineering.

\begin{table}[htbp]
\centering
\caption{Extended anomaly subtype detection (39 networks; TP/FN counts; FP tracking not available in current evaluation; source: \texttt{paper\_e8\_e12.json})}
\label{tab:confusion}
\begin{tabular}{@{}lcccc@{}}
\toprule
Subtype & TP & FN & R (\%) & Layer \\
\midrule
Grounding fault & 78 & 0 & 100.0 & Rule+SE \\
Clock drift & 36 & 0 & 100.0 & SE \\
Voltage regulation & 43 & 0 & 100.0 & Rule+SE \\
Topo interruption & 90 & 1 & 98.9 & Rule \\
Stale data & 67 & 5 & 93.1 & SE \\
Topo obfuscation & 57 & 17 & 77.0 & Rule+GNN \\
Load shift & 58 & 18 & 76.3 & SE \\
Reverse power flow & 104 & 48 & 68.4 & SE+GNN \\
Parameter error & 50 & 32 & 61.0 & SE \\
Protection misconfig & 45 & 32 & 58.4 & Rule+SE \\
Virtual/faulty conn.\ & 109 & 8 & 93.2 & SE+GNN \\
Telemetry mismatch & 68 & 8 & 89.5 & SE \\
Measurement outlier & 33 & 2 & 94.3 & SE \\
Trafo tap fault & 30 & 26 & 53.6 & SE \\
Measurement bias & 8 & 27 & 22.9 & SE \\
Voltage collapse & 9 & 97 & 8.5 & SE \\
Signal mismatch & 1 & 17 & 5.6 & Rule \\
Impedance degrad.\ & 1 & 113 & 0.9 & SE \\
Model mismatch (ext.) & 0 & 38 & 0.0 & --- \\
Communication loss & 0 & 104 & 0.0 & --- \\
Ghost topology & 0 & 59 & 0.0 & --- \\
Duplicate meas.\ & 0 & 74 & 0.0 & --- \\
Harmonic pollution & 0 & 74 & 0.0 & --- \\
DG intermittent & 0 & 51 & 0.0 & --- \\
Branch contingency & 0 & 114 & 0.0 & --- \\
\midrule
\textbf{Overall (25)} & \textbf{937} & \textbf{1\,057} & \textbf{47.0} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{RQ10: Realistic SCADA Validation with Real Load Profiles}

To evaluate the framework under realistic measurement conditions, we validate on 10 networks (6 SimBench~\cite{simbench2020} and 4 PandaPower~\cite{pandapower2018}) using real hourly load profiles from the Open Power System Data (OPSD)~\cite{opsd2020} ENTSO-E Transparency Platform (Germany, 2015, 167 hourly data points). Each network's loads are scaled with the real load profile, and realistic SCADA noise is applied ($\sigma_V = 0.005$\,p.u., $\sigma_P = 2\%$). Anomalies are injected at 8\% rate across three types (MTI, TI, SMC).

\begin{table}[htbp]
\centering
\caption{Realistic SCADA validation: 10 networks with OPSD real load profiles (source: \texttt{realistic\_scada\_validation.json})}
\label{tab:realistic_scada}
\begin{tabular}{@{}lcccc@{}}
\toprule
Method & R (\%) & P (\%) & F1 (\%) & T (s) \\
\midrule
Z-score          & 0.0  & 0.0  & 0.0  & $<$0.001 \\
LAV Robust SE    & 51.2 & 20.4 & 27.3 & $<$0.001 \\
Isolation Forest~\cite{isolation_forest2008} & 56.6 & 40.8 & 47.4 & 0.099 \\
Elliptic Envelope~\cite{rousseeuw1999fast} & 40.2 & 29.0 & 33.7 & 0.054 \\
\textbf{Proposed} & \textbf{78.1} & \textbf{44.8} & \textbf{55.9} & \textbf{0.94} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:realistic_scada} presents the results. The standalone baselines achieve 40--57\% recall on realistic SCADA data, confirming that measurement-only methods have limited detection capability even with real load profiles. The LAV Robust SE achieves moderate recall (51.2\%) but extremely low precision (20.4\%), flagging too many false positives. Isolation Forest achieves the highest standalone recall (56.6\%) but remains far below the full framework's 78.1\%. These results validate that the three-layer hybrid architecture's advantage is not an artifact of synthetic data---it persists under realistic measurement conditions with real-world load variability, measurement noise, and temporal patterns.

\textbf{Note.} The baseline results in Table~\ref{tab:realistic_scada} use simplified standalone implementations. The proposed framework's 78.1\% result uses the full three-layer pipeline with cross-layer integration, correction, and verification. The comparison demonstrates that domain-knowledge integration provides substantial gains over purely statistical approaches.
...
Four directions warrant investigation: (i)~three-phase unbalanced state estimation for heavily unbalanced feeders; (ii)~validation on real-world SCADA data with naturally occurring anomalies; (iii)~harder anomaly benchmarks exercising cross-layer complementarity; and (iv)~parallel partition processing for real-time monitoring of networks exceeding 1\,000 buses. The 46.8\% recall on extended types (RQ7) indicates significant room for improvement through real-world training. The open-source implementation and benchmarks are available at \texttt{https://github.com/secret-justice/topology-anomaly-detection}.
...
\item \textbf{OPSD}~\cite{opsd2020}: Open Power System Data---hourly electricity load, generation, and price data from ENTSO-E Transparency Platform. Freely available at \texttt{https://open-power-system-data.org}.
\end{itemize}
